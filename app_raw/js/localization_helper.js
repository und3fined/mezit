...
(function(exports) {
  ...
  function localizeDateTime(isTimeFormatChanged) {
    let dateTimeElements = document.querySelectorAll(isTimeFormatChanged ?
    ...
    Array.forEach(dateTimeElements, function(element) {
      ...
      // time format setting
      let formatL10nId = element.dataset.l10nDateFormat;
      ...
      let localeData = Utils.date.format.localeFormat(
      ...
      if (element.hasAttribute('data-l10n-id') &&
        ...
        let l10nAttrs = navigator.mozL10n.getAttributes(element);
  ...
  function localizePlaceholders() {
    let placeholderElements = document.querySelectorAll('.js-l10n-placeholder');
    Array.forEach(placeholderElements, function(element) {
      if (element.tagName === 'INPUT') {
        element.setAttribute('placeholder',
          navigator.mozL10n.get(Utils.camelCase(element.id) + '_placeholder'));
      } else {
