...
(function(exports) {
  ...
  function Recipient(opts) {
    ...
    this.className = 'recipient navigable';
  ...
  Recipients.prototype.add = function(entry) {
    ...
    */
    ...
    // If there are two recipients, alert the user select group or not.
    if (list.length === 2 && Startup.hasGroup) {
  ...
  Recipients.View.prototype.handleEvent = function(event) {
    ...
    let [isPreventingDefault, isAcceptedRecipient, isEdittingRecipient,
         isDeletingRecipient, isLookupable] =
        [false, false, false, false, false];
    let target = event.target;
    let keyCode = event.keyCode;
    let editable = 'false';
    let lastElement = view.inner.lastElementChild;
    let typed, recipient, length, toDelete;
