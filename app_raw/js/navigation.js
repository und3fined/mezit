...
(function(window) {
  ...
  let Navigation = {
    ...
    panels: {
      ...
      'settings-view': {
        behaviour: 'SettingsUI',
        wrapperPosition: 'right',
        container: 'messaging-settings'
      },
      'search-message-view': {
        behaviour: 'SearchMessageUI',
        wrapperPosition: 'right',
        container: 'searchMessage'
      }
    },
    ...
    isCurrentPanel: function n_isCurrentPanel(panel, args) {
      ...
      if (panel !== this.currentPanel.panel) {
        return false;
      }
      ...
      let currentArgs = this.currentPanel.args || {};
    ...
    isDefaultPanel: function n_isDefaultPanel() {
      let panelName = this.getPanelName();
    ...
    toPanel: function n_toPanel(panel, args) {
      ...
      let notReadyNavigate = (panel !== this.defaultPanel && !this.isReady);
      ...
      transitionArgs.meta = {
        ...
        prev: this.currentPanel
      };

      let nextPanelInfo = this.panels[panel];
      let nextPanelContainer = document.getElementById(nextPanelInfo.container);
      let nextPanelObject = this.panelObjects[nextPanelInfo.behaviour];
      ...
      let currentPanelObject;
      let currentPanelContainer;
      if (this.currentPanel) {
        let currentPanelInfo = this.panels[this.currentPanel.panel];
      ...
      let prevPanel = this.currentPanel;
      promise = promise.then(() => {
      ...
      promise = promise.then(
        ...
        if (NavigationMap.onPanelChanged) {
          NavigationMap.onPanelChanged(
            (prevPanel ? prevPanel.panel : null), panel, args);
        }
