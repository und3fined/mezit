...
(function(exports) {
  ...
  let ThreadUI = {
      ...
      Utils.findBlockNumbers('number', message.sender).then((result) => {
        if (result.length === 0) {
          ...
          if (!this.isCurrentThread(message.threadId)) {
             /* << [BTS-675] BDC kanxj 201900408 add for DUT can't continue typing normally when receive a SMS */
             // If in the panel of composer recovery focus
            if (Navigation.isCurrentPanel('composer')) {
              this.recoveryFocus();
            }
            /* >> [BTS-675] */
     ...
     */
    realSaveDraft: function thui_saveDraft(opts, backData) {
    ...
    saveDraft: function(opts, callback) {
      ...
      Utils.exceedLimitSpace().then((exceed) => {
        ...
        if (exceed) {
          ...
          this.realSaveDraft(opts, backData);
        }
      });
    },
