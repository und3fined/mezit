...
var SoftkeyPanel = function(actions, optionMenuCallback, area) {
  ...
  this.keyDownHandler = function(e) {
    ...
    switch (e.key) {
      ...
      case "SoftRight":
        ...
        if (this.buttonRsk.getAttribute("isNoResponse") == "yes") {
  ...
  function setSoftkeys() {
    function getSkHtml(item) {
      ...
      try {
        return item.icon
          ? `<span data-name="${name}">${item.iconMixedText ? name : ""}</span>
    ...
    function updateSoftKeys(actions, handlers, button, index) {
      let item = actions[index];
      handlers.set(button, item);
      button.innerHTML = getSkHtml(item);
      if (item.l10nId === undefined) {
        if (item.icon) {
    ...
    if (this.rskIndex !== -1) {
      ...
      this.buttonRsk.innerHTML =
        navigator.mozL10n.get("softkey-options") || "Options";
      needDisable = false;
    } else {
      this.buttonRsk.innerHTML = "";
...
SoftkeyPanel.prototype.disableRightKey = function() {
  this.buttonRsk.setAttribute("isNoResponse", "yes");
  this.handlers.set(this.buttonRsk, this.actions[2]);
};
SoftkeyPanel.prototype.enableRightKey = function() {
  this.buttonRsk.removeAttribute("isNoResponse");
  this.menuStartIndex = 3;
};
...
function getInfoFromButton(targetElement, self) {
  ...
  if (childElement) {
    ...
    info.options = { name: targetElement.textContent, icon: "" };
  }
  return info;
}
