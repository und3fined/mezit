(function(window, undefined) {
  ...
  var PropertiesParser = {
    ...
    setEntityValue: function(id, attr, key, rawValue, ast) {
      var pos, v;
      var value =
        rawValue.indexOf("{{") > -1 ? this.parseString(rawValue) : rawValue;
      ...
      if (attr) {
        ...
        if (key) {
          if (typeof ast[pos][attr] === "string") {
            ast[pos][attr] = { $x: this.parseIndex(ast[pos][attr]), $v: {} };
          }
  ...
  function resolveSelector(args, env, expr, index) {
    ...
    var argValue = index[1]
      ? resolveIdentifier(args, env, index[1])[1]
      : undefined;
  ...
  function Locale(id, ctx) {
    ...
    this.entries.__plural = getPluralRule(
      this.isPseudo ? this.ctx.defaultLocale : id
    );
  }
  var bindingsIO = {
    extra: function(id, ver, path, type, callback, errback, sync) {
      ...
      navigator.mozApps
        .getLocalizationResource(id, ver, path, type)
  ...
  Locale.prototype.addAST = function(ast) {
    var createEntry = this.isPseudo
      ? createPseudoEntry.bind(this)
      : Resolver.createEntry;
  ...
  function formatEntity(args, entity) {
    ...
    var locals = rv[0];
    var value = rv[1];
    var formatted = { value: value, attrs: null, overlay: locals.overlay };
    if (entity.attrs) {
  ...
  function formatAsync(fn, id, args) {
    return this._ready
      .then(getWithFallback.bind(this, id))
      .then(fn.bind(this, args), reportMissing.bind(this, id));
  }
  ...
  function negotiate(available, requested, defaultLocale) {
    if (
      available.indexOf(requested[0]) === -1 ||
  ...
  Context.prototype.requestLocales = function requestLocales() {
    ...
    var supported = negotiate(
      this.availableLocales.concat(reqPseudo),
      requested,
      this.defaultLocale
    );
  ...
  navigator.mozL10n = {
    ...
    _config: { appVersion: null, localeSources: Object.create(null) },
  ...
  navigator.mozL10n.ctx.addEventListener(
    "notfounderror",
  ...
  var readyStates = { loading: 0, interactive: 1, complete: 2 };
  ...
  if (window.document) {
    isPretranslated =
      !PSEUDO_STRATEGIES.hasOwnProperty(navigator.language) &&
  ...
  function initResources() {
    var meta = {};
    var nodes = document.head.querySelectorAll(
    ...
    if (navigator.mozApps && navigator.mozApps.getAdditionalLanguages) {
      additionalLanguagesPromise = navigator.mozApps
  ...
  function onMetaInjected(node, meta) {
    switch (node.getAttribute("name")) {
      case "availableLanguages":
        meta.availableLanguages = splitAvailableLanguagesString(
